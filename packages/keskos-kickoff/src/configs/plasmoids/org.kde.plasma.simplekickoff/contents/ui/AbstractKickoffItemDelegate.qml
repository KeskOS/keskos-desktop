...
T.ItemDelegate {
    ...
    readonly property bool currentItemSelected: view && view.currentIndex === index
    readonly property bool currentItemHovered: mouseArea.containsMouse
    ...
    background: Item {
        ...
        Rectangle {
            anchors.fill: parent
            color: root.currentItemSelected
                ? keskStyle.selectedFillColor
                : (root.currentItemHovered ? keskStyle.hoverFillColor : "transparent")
            border.width: root.currentItemSelected || root.currentItemHovered ? 1 : 0
            border.color: root.currentItemSelected ? keskStyle.accentBrightColor : keskStyle.borderColor
            radius: keskStyle.panelCornerRadius
        }

        Rectangle {
            visible: root.currentItemSelected
            anchors.fill: parent
            anchors.margins: 1
            color: "transparent"
            border.width: 1
        ...
        Rectangle {
            visible: root.isCategoryListItem && root.currentItemSelected
