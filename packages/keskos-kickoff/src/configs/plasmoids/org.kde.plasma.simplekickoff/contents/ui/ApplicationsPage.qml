...
BasePage {
    ...
    readonly property real fixedSidebarWidth: Kirigami.Units.gridUnit * 13
    
    sideBarComponent: KickoffListView {
        id: sidebar
        focus: true
        model: kickoff.rootModel
        implicitWidth: Math.max(root.fixedSidebarWidth, root.preferredSideBarWidth) + kickoff.backgroundMetrics.leftPadding + kickoff.backgroundMetrics.rightPadding
    ...
    contentAreaComponent: VerticalStackView {
        ...
        readonly property string preferredAllAppsViewObjectName: Plasmoid.configuration.applicationsDisplay === 0 ? "applicationsGridView" : "applicationsListView"
        readonly property Component preferredAllAppsViewComponent: Plasmoid.configuration.applicationsDisplay === 0 ? applicationsGridViewComponent : applicationsListViewComponent
